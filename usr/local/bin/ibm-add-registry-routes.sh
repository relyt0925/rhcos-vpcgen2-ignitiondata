...
if [[ "$PRIVATE_SERVICE_ENDPOINT_ENABLED" != "true" ]]; then
	echo "no need to setup haproxy registry routes"
	exit 0
fi
...
if grep "169.254.2.7" /etc/hosts; then
	# shellcheck disable=SC2154
...
ip addr add 169.254.2.8/32 brd 169.254.2.8 scope host dev lo
ip route add 169.254.2.8/32 dev lo scope link src 169.254.2.8
if grep "169.254.2.8" /etc/hosts; then
	# shellcheck disable=SC2154
	sed -i "s/^169.254.2.8.*/169.254.2.8 jp2.icr.io/g" /etc/hosts
else
	echo "169.254.2.8 jp2.icr.io" >>/etc/hosts
fi

ip addr add 169.254.2.9/32 brd 169.254.2.9 scope host dev lo
ip route add 169.254.2.9/32 dev lo scope link src 169.254.2.9
if grep "169.254.2.9" /etc/hosts; then
	# shellcheck disable=SC2154
	sed -i "s/^169.254.2.9.*/169.254.2.9 ca.icr.io/g" /etc/hosts
else
	echo "169.254.2.9 ca.icr.io" >>/etc/hosts
fi

ip addr add 169.254.2.10/32 brd 169.254.2.10 scope host dev lo
ip route add 169.254.2.10/32 dev lo scope link src 169.254.2.10
if grep "169.254.2.10" /etc/hosts; then
	# shellcheck disable=SC2154
	sed -i "s/^169.254.2.10.*/169.254.2.10 br.icr.io/g" /etc/hosts
else
	echo "169.254.2.10 br.icr.io" >>/etc/hosts
fi
